...
const EMPTY = () => ({ candidates: [], staff: [], reviews: [], sessions: [], evaluations: [], interviews: [], decisions: [], accommodations: [], details: {}, settings: {}, outcomes: [], audit: [], transcripts: [], jobs: [] });
...
export class LiveStore {
  ...
  async loadAll() {
    ...
    this.raw = {
      ...
      outcomes: rows(10), audit: rows(11), transcripts: rows(12), jobs: rows(13)
    };
    // when the question bank was last edited — profiles compare it with their report's scored_at
    const be = rows(9).find(x => x.key === 'bankEdits');
    if (be && be.updated_at) this.raw.settings.bankEditsUpdatedAt = be.updated_at;
    this.view = null;
    return this.raw;
  }
  start(onChange) {
  ...
  createCandidate(f) {
    return this._run(sb.from('candidates').insert({
      ...
      loc: f.loc || '', school: f.school || '', program: f.program || '', linkedin: f.linkedin || '', notes: f.notes || '', job_id: f.jobId || null, created_by: this.me && this.me.id
    }).select('*').single());
  }
  // ---- jobs / pipeline board ----
  createJob(f) { return this._run(sb.from('jobs').insert({ title: f.title, program: f.program || '', created_by: this.me && this.me.id }).select('*').single()); }
  updateJob(id, patch) { return this._run(sb.from('jobs').update(patch).eq('id', id)); }
  async findOrCreateJob(title, program) {
    const p = String(program || '').trim();
    const hit = (this.raw.jobs || []).find(j => j.title === title && (j.program || '') === p);
    if (hit) return hit;
    const row = await this.createJob({ title, program: p });
    this.raw.jobs = [...(this.raw.jobs || []), row]; this.view = null;
    return row;
  }
  ...
  viewData(D) {
    ...
    const candidates = R.candidates.filter(c => !c.archived).map(c => {
      ...
      const inviteState = !c.invite_sent_at ? (source === 'manual' ? 'Added manually \u00b7 no link sent' : 'Link not sent') : expired ? 'Link expired' : c.opened_at ? (source === 'self' ? 'Applied via link \u00b7 opened' : 'Opened') : 'Sent \u00b7 not opened';
      ...
      // pipeline board: the stage staff set, or one derived from where the candidate actually is
      const firstCall = T.filter(t => /^First call/i.test(t.kind || '')).sort((a, b) => new Date(b.created_at).getTime() - new Date(a.created_at).getTime());
      const callGrade = (firstCall.find(t => t.grade) || {}).grade || '';
      const autoStage = P.withdrawn ? 'Not moving forward' : dec ? (dec.decision === 'Advance' ? 'Offer' : dec.decision === 'Hold' ? 'Combine' : 'Not moving forward') : (E.length || ses) ? 'Combine' : done.s3 ? 'Assessment' : firstCall.length ? 'First call' : 'Applied';
      const reminders = (c.reminder1_at ? 1 : 0) + (c.reminder2_at ? 1 : 0);
      const stageLabelFull = stageLabel + (reminders && !done.s3 && !P.infoDone ? ' \u00b7 reminded \u00d7' + reminders : '');
      return {
        ...
        stage, stageLabel: stageLabelFull, readiness, r1: r1avg, r2: r2avg, agree, agreeMax, agreeComp, sjt: rp ? rp.overall : null, wdi: null, appEv,
    ...
    this.view = { candidates, users, sessions, accoms, applications, decisions, funnel, hires, audit, roles, transcripts, jobs };
    return this.view;
  }
}
