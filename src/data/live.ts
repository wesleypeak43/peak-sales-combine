...
const EMPTY = () => ({ candidates: [], staff: [], reviews: [], sessions: [], evaluations: [], interviews: [], decisions: [], accommodations: [], details: {}, settings: {}, outcomes: [], audit: [] });
...
export class LiveStore {
  ...
  async loadAll() {
    ...
    this.raw = {
      ...
      outcomes: rows(10), audit: rows(11)
    };
    this.view = null;
    return this.raw;
  }
  ...
  createCandidate(f) { return this._run(sb.from('candidates').insert({ name: f.name, email: f.email, phone: f.phone || '', role: f.role, created_by: this.me && this.me.id }).select('*').single()); }
  sendInvite(candidateId) { return this.api('/api/invite', { candidateId }); }
  extendInvite(candidateId) { return this.api('/api/invite', { candidateId, mode: 'extend' }); }
  ...
  insertSession(s) { return this._run(sb.from('sessions').insert({ candidate_id: s.candidate_id, when_txt: s.when_txt, e1: s.e1 || null, e2: s.e2 || null, link: s.link || '', created_by: this.me && this.me.id })); }
  ...
  viewData(D) {
    ...
    const candidates = R.candidates.filter(c => !c.archived).map(c => {
      const P = c.progress || {}, done0 = P.done || {};
      ...
      const inviteState = !c.invite_sent_at ? 'Link not sent' : expired ? 'Link expired' : c.opened_at ? 'Opened' : 'Sent \u00b7 not opened';
      let stage = 1, stageLabel = 'Invited \u00b7 ' + inviteState.toLowerCase();
      if (c.opened_at && !done.s1) stageLabel = 'Opened \u00b7 stage 1 in progress';
      if (done.s1 && !done.s2) stageLabel = 'In progress \u00b7 application';
      if (done.s2) { stage = 2; stageLabel = 'Application submitted \u00b7 in review'; }
      if (rv) { if (rv.outcome === 'advanced') { stage = 3; stageLabel = 'Application advanced \u00b7 sales decisions pending'; } else { stage = 2; stageLabel = 'Not advanced \u00b7 application stage'; } }
      if (done.s3) { stage = 4; stageLabel = 'Sales decisions complete \u00b7 awaiting combine'; }
      ...
      const appEv = rv ? rv.level : (done.s2 ? 'In review' : '\u2014');
      const ivAvg = mean(I.map(i => mean(Object.values(i.scores || {}))));
      const strength = (hasScores || rp || rv) ? { sim: lvl(r1avg), repitch: lvl(r2avg), case: done.s5b ? 'Submitted' : '\u2014', interview: lvl(ivAvg), sjt: rp ? bandLvl(rp.band) : '\u2014', app: appEv, wdi: '\u2014' } : null;
      ...
      const open = (rp && rp.followUps && rp.followUps.length) ? rp.followUps[0].q : dec ? '\u2014' : (stage >= 4 && !ses) ? 'Schedule the combine.' : (stage === 2 && !rv) ? 'Review the application.' : '\u2014';
      const resendRequested = !!c.resend_requested_at && (!c.invite_sent_at || new Date(c.resend_requested_at) > new Date(c.invite_sent_at));
      return {
        id: c.id, name: c.name, email: c.email, phone: c.phone || '', anon: anonOf(c.id), role: c.role,
        loc: (P.app && P.app.loc) || '\u2014', school: c.email,
        ...
        resendRequested, done, progress: P, report: rp, scoredAt: c.scored_at, hasSession: !!ses, sessionId: ses ? ses.id : null,
        evalCount: E.length, evaluations: E, withdrawn: !!P.withdrawn, expiresAt: c.invite_expires_at, createdAt: c.created_at
      };
    });
    const candById = Object.fromEntries(candidates.map(c => [c.id, c]));
    const sessions = R.sessions.map(s => { const c = candById[s.candidate_id] || {}; return { id: s.id, candId: s.candidate_id, cand: c.name || '\u2014', role: c.role || '', when: s.when_txt, e1: s.e1, e2: s.e2, evals: short(s.e1) + ' + ' + short(s.e2), link: s.link || '\u2014', ver: s.ver || 'v1.2', status: s.status || 'Confirmed' }; });
    ...
    const funnel = [
      { label: 'Invited', n: candidates.length },
      { label: 'Application submitted', n: n(c => c.done.s2) },
      { label: 'Sales decisions complete', n: n(c => c.done.s3) },
    ...
    const roles = (D.roles || []).map(r => { const apps = candidates.filter(c => c.role === r.title); const st = !apps.length ? 'No candidates yet' : apps.some(c => c.stage >= 5) ? 'Combine week' : apps.some(c => c.stage >= 4) ? 'Scheduling combines' : apps.some(c => c.stage >= 2) ? 'Application review' : 'Invites out'; return { ...r, apps: apps.length, stage: st }; });
    this.view = { candidates, users, sessions, accoms, applications, decisions, funnel, hires, audit, roles };
    return this.view;
  }
}
